...
"""
This file is a lower fidelity model 
based off of the default example that will
hopefully run models a lot faster for debugging
...
h_refl = ConvectiveModel(mat=Flibe(name='flibe'),
                         h0=600 * units.watt / units.kelvin / (units.meter**2),
                         model='constant')

void_av = 0.4
r_reflin = 0.35 * units.meter
r_reflout = 1.05 * units.meter
m_flow = 976.0 * 0.3 * units.kg / units.seconds
r_shell = 1.5 / 100 * units.meter

h_mod = ConvectiveModel(
    mat=Flibe(name='filbe'),
    m_flow= m_flow,
