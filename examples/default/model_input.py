...
Fuel = Material(name='fuel',
                ...
                dm=DensityModel(a=10500.0 * 
                                units.kg / (units.meter**3),
                                model="constant")) 

Cool = LiquidMaterial(name='cool',
                    ...
                    dm=DensityModel(a=2413.2172 *
                                    units.kg / (units.meter**3),
                                    b=-0.488 * units.kg /
                                    (units.meter**3) / units.kelvin,
                                    model="linear"),
                    mu=ViscosityModel(a=1.16e-4 * 
                                      units.pascal * units.second,
                                      b=3755 * units.kelvin,
                                      model="exponential"))

ReflGraphite = Material(name='refl',
                        k=ConductivityModel(model='constant',
...
CoreGraphite = Material(name='core',
                        k=ConductivityModel(model='constant',
                                 a=0.26 * units.watt / (units.meter * units.kelvin)),
                        cp=1650.0 * units.joule / (units.kg * units.kelvin),
                        dm=DensityModel(a=1740. * 
...
GraphPebble = Material(name='graph_peb',
                        k=ConductivityModel(model='constant',
                                 a=0.26 * units.watt / (units.meter * units.kelvin)),
                        cp=1650.0 * units.joule / (units.kg * units.kelvin),
                        dm=DensityModel(a=1740. * 
